...
def get_args():
    parser = argparse.ArgumentParser(
        description="Given files of a specified type, their text is combined into one file.")
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        "-o", "--outdir", help="output file directory (default <dir>)")
    group.add_argument("-d", "--default", action="store_true",
                       help="use output directory specified in config.json")
    parser.add_argument("-f", "--fileoutname",
                        help="output file name (default: `--combined.<type>`")
